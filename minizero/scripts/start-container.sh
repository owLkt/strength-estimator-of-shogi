...
container_volume="-v .:/workspace -v /media/katiogoto/abb1e836-e529-4059-80be-167839ba757a:/data:ro"
container_argumenets="--name boring_torvalds"
...
echo "$container_tool run ${container_argumenets} --gpu all --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --network=host --ipc=host --rm -it ${container_volume} ${image_name}"
$container_tool run ${container_argumenets} --env CMAKE_PREFIX_PATH="/usr/local/libtorch/share/cmake/Torch:/usr/local/share/cmake/pybind11:/usr/local" --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --network=host --ipc=host --rm -it ${container_volume} ${image_name}