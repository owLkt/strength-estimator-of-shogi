...
class PolicyNetwork(nn.Module):
    def __init__(self, num_channels, channel_height, channel_width, action_size):
        ...
        #print("\n--- [PolicyNetwork Debug] ---")
        print(f"Passed num_channels: {num_channels}")
        print(f"Passed channel_height: {channel_height}")
        print(f"Passed channel_width: {channel_width}")
        print(f"Passed action_size: {action_size}")
        #print("------------------------------\n")
    ...
    def forward(self, x):
        ...
        x = self.bn(x)
        x = F.relu(x)
        x = x.reshape(-1, self.num_output_channels * self.channel_height * self.channel_width)
        x = self.fc(x)
        return x
